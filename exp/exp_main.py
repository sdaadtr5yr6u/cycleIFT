...
from models import Informer, LSTM, CNN, MLP, BiLSTM
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    ...
                    if self.args.model in ['LSTM', 'CNN', 'MLP', 'BiLSTM']:
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                pred = outputs.detach().cpu().numpy()  # torch.Size([32, 24, 7])
                true = batch_y.detach().cpu().numpy()  # torch.Size([32, 24, 7])
        ...
        for i in range(4):
            ...
            mae_i = np.mean(np.abs(preds[:,:,i] - trues[:,:,i]))
            print(f'Water quality indicator {i + 1} - MSE: {mse_i}, MAE: {mae_i}')
